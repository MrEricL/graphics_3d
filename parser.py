...
def parse_file( fname, edges, transform, screen, color ):
    ...
    while c < len(lines):
        ...
        if line == "sphere":
            generate_sphere(edges,float(args[0]), float(args[1]), float(args[2]),float(args[3]), 50)
        if line == "torus":
            generate_torus(edges,float(args[0]), float(args[1]), float(args[2]),float(args[3]), float(args[4]), 100)     
